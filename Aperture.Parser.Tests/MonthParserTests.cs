...
namespace Aperture.Parser.Tests
{
    ...
    public class MonthParserTests
    {
        ...
        {
            Assert.AreEqual(true, MonthParser.IsValidMonthString("2000-12"));
            Assert.AreEqual(true, MonthParser.IsValidMonthString("2000-01"));
            Assert.AreEqual(true, MonthParser.IsValidMonthString("102000-05"),
                "Cannot handle year strings longer than 4 digits.");
            Assert.AreEqual(false, MonthParser.IsValidMonthString("0000-01"),
                "Year string must be 1 or greater.");
            Assert.AreEqual(false, MonthParser.IsValidMonthString("200011"),
                "Accepts year and month string without hyphen in between.");
            Assert.AreEqual(false, MonthParser.IsValidMonthString("2000-17"),
                "Accepts month greater than 12.");
            Assert.AreEqual(false, MonthParser.IsValidMonthString("2000-00"),
                "Accepts month string less than 1.");
            Assert.AreEqual(false, MonthParser.IsValidMonthString("2000-01AAAAAA"));
        }
